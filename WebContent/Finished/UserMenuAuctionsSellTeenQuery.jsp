...
<head>
	...
	<%
		try{
			...
			int suggestive_themes =	Integer.parseInt(request.getParameter("suggestiveThemes"));
			int violence =	Integer.parseInt(request.getParameter("mediumViolence"));
			int crude_humor =	Integer.parseInt(request.getParameter("crudeHumor"));
			int minimal_blood =	Integer.parseInt(request.getParameter("mediumBlood"));
			int gambling =	Integer.parseInt(request.getParameter("gambling"));
			int strong_language =	Integer.parseInt(request.getParameter("mediumLanguage"));
			...
			if(!badInput){
				...
				" FROM `VideoGame` V, `Teen` T WHERE BINARY V.game_name = BINARY '" + game_name + "' AND " +
				"V.`genre` = '" + genre + "'  AND V.`platform` = '" + platform + "' AND V.`condition` = '" + condition + 
				"' AND V.`total_players` = '" + total_players + 
				"' AND T.`suggestive_themes` = '" + suggestive_themes +
				"' AND T.`violence` = '" + violence +
				"' AND T.`crude_humor` = '" + crude_humor +
				"' AND T.`minimal_blood` = '" + minimal_blood +
				"' AND T.`gambling` = '" + gambling +
				"' AND T.`strong_language` = '" + strong_language +
				"' AND V.`gameID` = T.`gameID`";		
				...
				if(!gameSearch.next()){
					...
					String teen_insert = "INSERT INTO `Teen`(`gameID`, `suggestive_themes`, `violence`, `crude_humor`, `minimal_blood`, `gambling`, `strong_language`)"
							+ " VALUES(?, ?, ?, ?, ?, ?, ?)";
					PreparedStatement create_teen = con.prepareStatement(teen_insert);
					create_teen.setInt(1, new_gameID);
					create_teen.setInt(2, suggestive_themes);
					create_teen.setInt(3, violence);
					create_teen.setInt(4, crude_humor);
					create_teen.setInt(5, minimal_blood);
					create_teen.setInt(6, gambling);
					create_teen.setInt(7, strong_language);
			...
			out.print("Error. Something went wrong in Teen go back to selling");
			out.print(ex);
		}
	%>
	<br>
